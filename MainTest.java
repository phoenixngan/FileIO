...
public class MainTest {
       ...
       do {
           ...
           try {
               ...
               if (option.equalsIgnoreCase("a")) {
                   ...
                   if (args.length > 0) {
                       fileNameSale = args[0];
                       fileForSale = new File(args[0]);
                   } else {
                       // if there is no command-line argument, send an error message stating no data can be written
                       throw new UnknownTransactionException("No command-line argument was found." +
                   ...
                   fileNameService = serviceSold + ".txt";
                   fileForService = new File(fileNameService);

                   try {
                       // check if the input file actually exists
                       if (!fileForSale.isFile()) {
                           throw new FileNotFoundException(fileNameSale);
                       }
                       ...
                       if (!alreadyWrittenToFile) {
                           writeToFile(fileNameSale, clientName, serviceSold, amountOfSale, dateOfEvent);
                       ...
                       if (!fileForService.isFile()) {
                           throw new FileNotFoundException(fileNameService);
                       }

                       // the service file exists; now read the data from the input file and write to the service file

                       readAndWriteToServiceFile(fileForSale, fileForService);
               ...
               PrintMessage.noFileTypeExistsError(".txt");
               ...
               // allow the user to escape the loop; reset the flags
               clientNameIsValid = false;
               serviceSoldIsValid = false;
               amountOfSaleIsValid = false;
               dateOfEventIsValid = false;
               ...
               // all other potential exceptions not accounted for
               PrintMessage.unexpectedExceptionError(error.getMessage());
    ...
    * @param dateOfEvent the sate of the event
    */
   public static void writeToFile(String fileName, String clientName, String serviceSold, String amountOfSale,
   ...
   public static void readAndWriteToServiceFile(File fileToRead, File fileToWrite) throws IOException {
       ...
       writeToFile(fileToWrite.getName(), parsedLine[0], parsedLine[1], parsedLine[2], parsedLine[3]);
   }

   /**
    * Reads from a file using the given ArrayList of files.
    *
       ...
       for (File fileToRead : arrayListOfFiles) {
           try (BufferedReader bReader = new BufferedReader(new FileReader(fileToRead))) {
               ...
               while ((line = bReader.readLine()) != null) {
                   retrievedData.append("\t").append(line.replace(";", " - "))
                           .append("\n");
               }
           }
       }
